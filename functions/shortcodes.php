...
function site_identity_func( $atts = array(), $content = null ) {
	...
	$logo_url = ($appolo_options['logo']['url']) ? $appolo_options['logo']['url'] : get_template_directory_uri(). '/images/logo.png';
	$logo_option = $appolo_options['logo-option'];
	$html = '';
...
function email_func( $atts = array(), $content = '' ) {	
	global $appolo_options;
	$contact_email = $appolo_options['contact-email'];
	$html = '';	
	$atts = shortcode_atts( array(
		'offset' => 0,
		'index' => 0,
...
function phone_func( $atts = array(), $content = '' ) {
	...
	if ($atts['index']) :
	    ...
	    $html .= preg_replace('/[^0-9]/', '', $appolo_options['contact-phone'][$i]);
	    $html .= '" >';
	    $html .= $appolo_options['contact-phone'][$i];  
	    $html .= '</a>';
	    $html .= '</span>';		
	else :
		foreach ($appolo_options['contact-phone'] as $phone) :
			if ($n > $atts['offset']) :
			    $html .= '<span class="phone-number">';
			    $html .= '<a class="phoneToShow" href="tel:';
...
function fax_func( $atts = array(), $content = '' ) {
	...
	if ($atts['index']) :
	    ...
	    $html .= preg_replace('/[^0-9]/', '', $appolo_options['contact-fax'][$i]);
	    $html .= '" >';
	    $html .= $appolo_options['contact-fax'][$i];  
	    $html .= '</a>';
	    $html .= '</span>';		
	else :
		foreach ($appolo_options['contact-fax'] as $fax) :
			if ($n > $atts['offset']) :
			    $html .= '<span class="fax-number">';
			    $html .= '<a class="faxToShow" href="tel:';
...
function address_func( $atts = array(), $content = '' ) {
	global $appolo_options;
	$contact_address = $appolo_options['contact-address'];
	$html = '';
	$atts = shortcode_atts( array(
		'offset' => 0,
		'map' => 0,
...
function social_menu_fnc( $atts = array(), $content = '' ) {
	...
	global $appolo_options;
	$html = '';
	$contact_social = $appolo_options['contact-social'];
	$contact_address = $appolo_options['contact-address'];
	$atts = shortcode_atts( array(
		'display' => 'inline',
		'title' => 0,
